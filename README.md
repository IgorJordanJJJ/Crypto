...
#### База данных
- **ClickHouse** - колоночная СУБД, оптимизированная для аналитических запросов. Выбрана по следующим причинам:
  - Высокая производительность для аналитических запросов
  - Отличное сжатие данных (важно для временных рядов)
  - Поддержка SQL и агрегаций
  - Масштабируемость для больших объемов данных

#### Источники данных
- **CoinGecko API** - бесплатный API для получения данных о криптовалютах
- **DefiLlama API** - API для получения данных о DeFi протоколах и TVL
...
- **PostgreSQL** - хранение метаданных Superset (пользователи, дашборды, настройки)
...
```bash
# Установка и запуск ClickHouse отдельно
...
4. Убедитесь в запуске ClickHouse
